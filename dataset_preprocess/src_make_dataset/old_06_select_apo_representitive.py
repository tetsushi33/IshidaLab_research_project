...
import argparse
#from IshidaLab_research_project.dataset_preprocess.src_make_dataset.modules import old_module_determining_pocket
from modules import old_module_determining_pocket
from modules import module_apo_grouping
...
def main(start_id, end_id):
    ...
    for protein_id in tqdm(range(start_id, end_id+1)): # protein_idといったら、今後はグループのidのことをさす
        ...
        apo_representitive = old_module_determining_pocket.determine_most_common_apo(protein_id, apo_holo_pairs_csv, apo_proteins_id_csv)
        ...
        holo_to_pocket_id, holo_to_pocket_centroids, holo_to_pocket_rmsd, merged_pockets, apo_pocket_loop_percentage, apo_pocket_missing_percentage = old_module_determining_pocket.determine_pocket_id_for_apo_a(
        ...
        apo_a_seq = old_module_determining_pocket.read_fasta(apo_a_fasta_path)
        ...
        if len(apo_proteins_id_csv[apo_proteins_id_csv['protein_id'] == protein_id]) > 1: # 同じidをのアポタンパク質が複数個あれば
            for index, apo_b_info in apo_proteins_id_csv[apo_proteins_id_csv['protein_id'] == protein_id].iterrows(): #代表でないアポを一つずつ処理
                ...
                apo_b_seq = old_module_determining_pocket.read_fasta(apo_b_fasta_path)
                apo_b_protein_path = f"../mmcif/apo/{apo_b_name}.cif"
                
                # アポタンパク質AとBのペアワイズアライメントを実行
                alignment = old_module_determining_pocket.align_sequences(apo_a_seq, apo_b_seq)
                print(f"Alignment result: \n{alignment}")
                # アポタンパク質AとBのFASTAとmmCIF間のマッピング生成
                fasta_to_pdb_mapping_a = old_module_determining_pocket.create_mapping_from_mmcif_to_fasta(apo_a_protein_path, apo_a_chain)
                fasta_to_pdb_mapping_b = old_module_determining_pocket.create_mapping_from_mmcif_to_fasta(apo_b_protein_path, apo_b_chain)
                print(f"Mapping from fasta to B: {fasta_to_pdb_mapping_b}")
                print(f"Mapping from fasta to A: {fasta_to_pdb_mapping_a}")
                apo_a_to_b_mapping = old_module_determining_pocket.create_mapping_from_alignment(alignment, fasta_to_pdb_mapping_a, fasta_to_pdb_mapping_b)
                print(f"Mapping from A to B: {apo_a_to_b_mapping}")

                # アポタンパク質Bのポケット位置を特定
                pocket_rmsd_b, pocket_centroids_b, apo_b_pocket_loop_percentage, apo_b_pocket_missing_percentage = old_module_determining_pocket.determine_pocket_id_for_apo_b(
