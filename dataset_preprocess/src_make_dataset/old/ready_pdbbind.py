...
similar_apo_proteins_file = os.path.join(csv_directory, "similar_apo_proteins.csv")
ligand_info_file = os.path.join(csv_directory, "ligand_info.csv")
...
def main():
    ...
    # エラーログファイルの準備
    if not os.path.exists("../result_csv_files/error_log.csv"):
    ...
    if os.path.exists(pocket_data_file): # pocket_data_file = ../csv_file/pocket_data.csv
        print("ポケットデータ作成済み --- ", pocket_data_file)
        with open(pocket_data_file, 'r') as f:
            ...
            print("num of no dominant chain : ", len(no_dominant_chains))
        ...
        with open(similar_apo_proteins_file, 'w') as f:
            ...
            for pdb_id, apo_list in similar_apo_proteins.items():
                for apo_pdb_id, apo_chain in apo_list:
                    writer.writerow([pdb_id, apo_pdb_id, apo_chain])
        print("結果を保存 -----> ", similar_apo_proteins_file)


    print("===============リガンド情報を取得===============")
    ligand_info_dict = {}

    if os.path.exists(ligand_info_file):
        print("リガンド情報データ作成済み --- ", ligand_info_file)
        with open(ligand_info_file, 'r') as f:
            reader = csv.reader(f)
            next(reader)  # skip header
            for row in reader:
    ...
    for pdb_id, apo_candidates in similar_apo_proteins.items():
        ...
        for apo in apo_candidates:
            ...
            if apo_pdb_id_lower != pdb_id_lower:  
                ligand_info = ligand_info_dict.get(apo[0], (None, None))
                if not ligand_info[0]:  # Check if there's no ligand
                    valid_apos.append(apo)
        if valid_apos:
            filtered_apo_proteins[pdb_id] = valid_apos


    print("Writing to apo_holo_pairs.csv...")

    # アポタンパク質とホロタンパク質のペアをCSVに保存
    with open(apo_holo_pairs_file, "w") as f:
        writer = csv.writer(f)
